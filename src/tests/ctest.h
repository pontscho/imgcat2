...
#define CTEST_IMPL_STRUCT(sname, tname, tskip, tdata, tsetup, tteardown)                                                                                                          \
	static struct ctest CTEST_IMPL_TNAME(sname, tname) CTEST_IMPL_SECTION = {                                                                                                     \
		#sname, #tname, { (ctest_nullary_run_func)CTEST_IMPL_FNAME(sname, tname) }, tdata, (ctest_setup_func *)tsetup, (ctest_teardown_func *)tteardown, tskip, CTEST_IMPL_MAGIC, \
	}

#ifdef __cplusplus

#define CTEST_SETUP(sname)    template <> void CTEST_IMPL_SETUP_FNAME(sname)(struct CTEST_IMPL_DATA_SNAME(sname) * data)

#define CTEST_TEARDOWN(sname) template <> void CTEST_IMPL_TEARDOWN_FNAME(sname)(struct CTEST_IMPL_DATA_SNAME(sname) * data)
...
#define CTEST_TEARDOWN(sname)                                                                                                    \
	static void CTEST_IMPL_TEARDOWN_FNAME(sname)(struct CTEST_IMPL_DATA_SNAME(sname) *__attribute__((unused)) data);             \
...
#define CTEST_IMPL_CTEST2(sname, tname, tskip)                                                                                                         \
	static struct CTEST_IMPL_DATA_SNAME(sname) CTEST_IMPL_DATA_TNAME(sname, tname);                                                                    \
	static void CTEST_IMPL_FNAME(sname, tname)(struct CTEST_IMPL_DATA_SNAME(sname) *__attribute__((unused)) data);                                     \
...
__attribute__((no_sanitize_address)) int ctest_main(int argc, const char *argv[])
{
	...
	if (argc == 2) {
		if (strchr(argv[1], ':')) {
			...
			size_t suite_len = sep - argv[1];
			static char suite_buf[256];
			if (suite_len >= sizeof(suite_buf)) {
				fprintf(stderr, "Suite name too long\n");
				return 1;
			}
			strncpy(suite_buf, argv[1], suite_len);
			suite_buf[suite_len] = 0;
			suite_name = suite_buf;
			test_name = sep + 1;
			filter = test_filter;
